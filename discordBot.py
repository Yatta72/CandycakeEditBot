...
discord_token             = os.environ.get("DISCORD_TOKEN") or config.get("discord_token")
...
async def check_donors():
    ...
    if not donor_guild_id or not donor_guild_id.isdigit():
        print("No valid donor guild ID configured, skipping donor check.")
        return
...
async def parse_command(message):
    ...
    async with message.channel.typing():
        ...
        if is_timeout is not True:
            await message.reply(f"The bot is cooling down, try again in {ceil(is_timeout)} seconds.")
            return
...
async def on_ready():
    ...
    # Update status based on server count
    async def update_status_loop():
        while True:
            try:
                guild_count = len(bot.guilds)
                status = discord.Status.dnd if guild_count >= 75 else discord.Status.online
                await bot.change_presence(status=status, activity=discord.Game(name=discord_tagline))
    ...
    botReady = True
    info("✅ CandycakeEditBot is ready!")
